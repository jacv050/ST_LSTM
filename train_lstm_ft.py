...
from model_lstm import SkeletonAction_FT as SkeletonAction
...
def main(args):
    ...
    data_loader2,_ = get_loader(args.data_dir, args.seq_len, args.batch_size,
                             shuffle=False, num_workers=args.num_workers, ds = args.ds, lbl2id = ds_class.lbl2id)
    ...
    #params = model.linear_ft.parameters()
    optimizer = torch.optim.Adam(params, lr=args.learning_rate)
    
    ft_fn = args.model_ft_fn
    model_dict = model.state_dict()
    if ft_fn:
        logging.info('Loading pretrained model from %s', ft_fn)
        pretrained_dict = torch.load(ft_fn)
    ...
    for epoch in range(args.num_epochs):
        ...
        for i_step, (lbl, data, length) in enumerate(data_loader):
            ...
            if i_step % args.eval_step == 0:
                ...
                correct_num = 0
                correct_num2 = 0
                for k_step, (lbl, data, length) in enumerate(data_loader2):
                    lbl = Variable(lbl)
                    data = Variable(data)
                    mask = torch.zeros(data.size(0), data.size(1))
                    for i,m in zip(length, mask):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_ft_fn', type=str, default = '',
                        help='path for saving trained models')
    # Model parameters
