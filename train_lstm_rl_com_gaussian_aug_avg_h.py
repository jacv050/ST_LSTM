...
from model_lstm_rl_complicated import SkeletonAction, ValueNetwork, PolicyNetwork, CoreClassification
...
def main(args):
    ...
    model_value = ValueNetwork(args.input_size, args.hidden_size, args.num_class, args.num_layers, dropout = args.dropout )
    model_policy = PolicyNetwork(args.input_size, args.hidden_size, args.num_action, dropout = args.dropout)
    ...
    for epoch in range(args.num_epochs):
        ...
        for i_step, (lbl, data, length) in enumerate(data_loader):
            ...
            h_tensor.resize_(data.size(0), args.hidden_size) 
            ...
            ht, ct, data_t = model_base( data[:,0,:], zero, init_hs, init_cs)
            data_trans.append(data_t)
            exp_reward = model_value(data_t.unsqueeze(1)).squeeze()
            exp_rewards.append(exp_reward)
          
            hs.append(ht[-1])
            action_prob = model_policy(data_t.unsqueeze(1))
            action_probs.append(action_prob)
            action = Categorical(action_prob)
            action = action.sample()
            actions.append(action)
            for j_step in range(1, data.shape[1]):
                ht, ct, data_t = model_base( data[:,j_step,:], actions[j_step-1].float(), ht, ct)
                data_trans.append(data_t)
                hs.append(ht[-1])
                n_data = torch.stack(data_trans, dim = 1)
                action_prob = model_policy(n_data)
                exp_reward = model_value(n_data).squeeze()
                exp_rewards.append(exp_reward)
            ...
            pred_lbl_avg = logits.max(dim = 1)[1]
            ...
            for i in range(len(hs)):
                h_ = torch.stack(hs[0:i+1], dim = 1)
                h_ = (h_ * mask[:,0:i+1].unsqueeze(2) ).sum(dim = 1) / mask[:,0:i+1].sum(dim = 1).unsqueeze(1)
                logits = model_c(h_)
                pred_lbl = logits.max(dim = 1)[1]
                reward = Variable((pred_lbl.data == lbl.data).float())
                rewards.append(reward)
            for j_step, (action, action_prob) in enumerate(zip(actions, action_probs)):
                # total reward.
                #target = sum(discount ** i * reward for i,t in enumerate(rewards[j_step:]))
                target = rewards[j_step]
                exp_reward = exp_rewards[j_step]
               
                #logging.info('exp_reward: %.4f, target: %.4f', exp_reward.mean().data[0], target.mean().data[0])
                exp_reward = exp_reward[torch.LongTensor(range(data.size(0))).cuda(), lbl.data]
            ...
            if i_step % args.eval_step == 0:
                ...
                for k_step, (lbl, data, length) in enumerate(eval_data_loader):
                    ...
                    for j_step in range(1, data.shape[1]):
                        ht, ct, data_t = model_base( data[:,j_step,:], action.float(), ht, ct)
                        hs.append(ht[-1])
                        data_trans.append(data_t)
                        n_data = torch.stack(data_trans, dim = 1)
                        action_prob = model_policy(n_data)
